...
export function CryptoWidget({ className }: { className?: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/crypto?ids=bitcoin,ethereum,solana`, {
          cache: "no-store",
        });
        if (res.ok) {
  ...
  function sentimentBg(change?: number): string {
    ...
    if (change > 2)
      return "bg-gradient-to-b from-emerald-100/60 to-emerald-50/30 dark:from-emerald-900/30 dark:to-emerald-900/10";
    if (change > 0)
      return "bg-gradient-to-b from-emerald-50/60 to-emerald-50/20 dark:from-emerald-900/20 dark:to-emerald-900/5";
    if (change < -2)
      return "bg-gradient-to-b from-rose-100/60 to-rose-50/30 dark:from-rose-900/30 dark:to-rose-900/10";
