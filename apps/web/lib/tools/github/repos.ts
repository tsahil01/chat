...
export const listGitHubRepos = tool({
  ...
  execute: async ({ maxResults, type, sort }) => {
    try {
      ...
      const repos = await makeGitHubRequestWithFallback(
        `/user/repos?type=${type}&sort=${sort}&per_page=${maxResults}`,
      );
...
export const createGitHubRepo = tool({
  ...
  inputSchema: z.object({
    ...
    autoInit: z
      .boolean()
      .default(true)
      .describe("Whether to initialize with README"),
  }),
...
export const getGitHubRepoInfo = tool({
  ...
  execute: async ({ owner, repo }) => {
    try {
      // Try authenticated first, fallback to public if no auth
      const repoData = await makeGitHubRequestWithFallback(
        `/repos/${owner}/${repo}`,
      );
